...
#
#     elif lan == "ru":
#         prompt = f"Тема: {topic}. Пункт: {band}. Напишите краткий и понятный текст по данному пункту (750-850 символов). Не используйте специальные символы (** ## - *). Только обычный текст без заголовков."
#
#     elif lan == "en":
...
#
#
# def add_title_slide(prs, bg_bytes, title, author,lan):
#     slide = prs.slides.add_slide(prs.slide_layouts[6])
#     slide.shapes.add_picture(BytesIO(bg_bytes), 0, 0, width=prs.slide_width, height=prs.slide_height)
#
...
#     add_title_slide(prs, bg, topic, author,lan)
#
#
...
#         elif lan == "ru":
#             prompt = (
#                 f"'{topic}' mavzusi uchun rus tilida {slide_count} ta reja bandini generatsiya qiling. "
#                 "Har bir band yangi qatorda bo‘lsin va 5-7 so‘zdan iborat bo‘lsin."
#             )
#         elif lan == "en":
#             prompt = (
#                 f"Generate {slide_count} outline points for the topic '{topic}'. "
...
user_balans=db.select_user(tg_user=5528965178)
...
# from loader import db
#
# db.add_shablon(tg_id=1, institut=1, ism_fam=1, saxifa_soni=1, til=1)
