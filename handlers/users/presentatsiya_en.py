...
# -------- STATES --------
class PresState__en(StatesGroup):
    language = State()
    topic = State()
    author = State()
    bg = State()
...
class ImageCollectState__en__en(StatesGroup):
    collecting = State()
...
@router.callback_query(F.data=='taqdimot_en')
async def start(callback: CallbackQuery, state: FSMContext):
    olish=db.select_shablon_pre(tg_id=callback.from_user.id,til='en')
    if olish:
        ...
        if int(summa[3]) >= tekshir:
            ...
            print(ol, 'ol')
            await callback.message.answer("Enter the topic of the presentation:")
            await state.set_state(PresState_shablon__en.topic)
        else:
...
f"Use the /bye command to top up your account.")
        ...
        if int(summa[3]) >= tekshir:
            ...
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🇺🇿 O'zbekcha", callback_data="langpreen_uz")],
                [InlineKeyboardButton(text="🇷🇺 Русский", callback_data="langpreen_ru")],
                [InlineKeyboardButton(text="🇬🇧 English", callback_data="langpreen_en")]
            ])
            await callback.message.answer("Select presentation language:", reply_markup=keyboard)
            await state.set_state(PresState__en.language)
        else:
            text = (f"💰Your balance is: {pul[1]} soums\n\n"
f"Use the /bye command to top up your account.")
            await callback.message.answer(text, parse_mode="Markdown")

@router.callback_query(StateFilter(PresState__en.language), F.data.startswith("langpreen_"))
async def set_language(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        pass
    await callback.message.answer("📌 Enter the topic of the presentation:")
    await state.set_state(PresState__en.topic)

@router.message(StateFilter(PresState__en.topic))
async def get_topic(msg: Message, state: FSMContext):
    user_data[msg.from_user.id] = {"topic": msg.text}
    await msg.answer("✍️ Enter the author's name:")
    await state.set_state(PresState__en.author)

@router.message(StateFilter(PresState__en.author))
async def get_author(msg: Message, state: FSMContext):
    ...
    await bot.send_photo(
        ...
        photo=photo,
        caption="📸 Choose a presentation background (1-10)",
        reply_markup=inline_buttons
    )
    await state.set_state(PresState__en.bg)

@router.callback_query(StateFilter(PresState__en.bg), F.data.startswith("bg_"))
async def handle_bg_selection(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        try:
            ...
            pass
        await state.set_state(PresState__en.reja_mode)
        await callback.message.answer("📄 How many slides (number only):")
        await state.set_state(PresState__en.slide_count)
        await state.update_data(reja_mode="ai")
    except Exception as e:
        logging.error(f"Background load error: {e}")
        await callback.answer("❌ The background could not be loaded, please try again.", show_alert=True)

@router.message(StateFilter(PresState__en.bg_upload), F.photo)
async def handle_uploaded_bg(msg: Message, state: FSMContext):
    ...
    await state.set_state(PresState__en.reja_mode)
    await msg.answer("📄 How many slides (number only):")
    await state.set_state(PresState__en.slide_count)
    await state.update_data(reja_mode="ai")

@router.message(StateFilter(PresState__en.slide_count))
async def get_slide_count(msg: Message, state: FSMContext):
    summa = db.select_summa()
    try:
        if not msg.text.isdigit():
            await msg.answer("❌ Please enter a number only!")
            return

        requested_slide_count = int(msg.text)
        if requested_slide_count < 5:
            await msg.answer("❌The number of slides must be at least 5!")
            return
        if requested_slide_count > 50:
            await msg.answer("❌ The number of slides cannot exceed 50!")
            return
...
f"Use the /bye command to top up your account."
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Image selection", callback_data="img_select_preen")],
            [InlineKeyboardButton(text="♻️ Automatic pictures", callback_data="img_auto_preen")],
            [InlineKeyboardButton(text="🚫 Without picture", callback_data="img_none_preen")]
        ])

        a=await msg.answer(
            "📄 Number of slides accepted.\n\n"
            "✅ *Image selection* — you submit your own images related to the topic (up to 7) and only these images will be used.\n"
            "♻️ *Automatic images* — the bot loads relevant images from the Internet.\n"
            "🚫 *No images* — the presentation does not use images.",
            reply_markup=kb,
            parse_mode="Markdown"
        )
...
# --- Rasm tanlash rejimi ---
@router.callback_query(F.data == "img_select_preen")
async def select_images(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Finish", callback_data="img_finish_preen")],
        [InlineKeyboardButton(text="♻ Delete pictures", callback_data="img_reset_preen")]
    ])
    await callback.message.answer(
        "📤 Submit photos related to the topic (up to 7).\n"
"When you are ready — press *Finish*.\n"
"If you make a mistake — start over by *Clear Photos*.\n"
"🌃 Photos: 0/7",
        reply_markup=kb,
        parse_mode="Markdown"
    )
    await state.set_state(ImageCollectState__en__en.collecting)

@router.message(StateFilter(ImageCollectState__en__en.collecting), F.photo)
async def collect_images(msg: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Заканчивать", callback_data="img_finish_preen")],
        [InlineKeyboardButton(text="♻ Удалить фотографии", callback_data="img_reset_preen")]
    ])
    await msg.answer(f"🌃 Rasmlar: {len(user_images[uid])}/7", reply_markup=kb)

@router.callback_query(F.data == "img_reset_preen")
async def reset_images(callback: CallbackQuery):
    ...
    clear_user_images(uid, delete_files=True)
    await callback.message.answer(
        "♻ 0 photos have been uploaded. You can resend.\n🌃 Photos: 0/7"
    )

@router.callback_query(F.data == "img_finish_preen")
async def finish_images(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="✅ Preparation", callback_data="tayyorlash_preen"),
                InlineKeyboardButton(text="🚫 Rejection", callback_data="rad_etish_preen"),
                InlineKeyboardButton(text="✏️ Change", callback_data="ozgartirish_preen")
            ]
        ]
    )
...
f"Language: {lan}\n\n"
f"• If the information is correct, click '✅ Prepare'!\n"
f"• Click '✏️ Edit' to change\n"
f"• Click '🚫 Reject' to cancel",
                             reply_markup=kb, photo=rasim
                             )
    await state.update_data({"m_id": a.message_id})
    await state.set_state(PresState__en.tanlov)
    await callback.answer()

# --- Rasmsiz rejim ---
@router.callback_query(F.data == "img_none_preen")
...
@router.callback_query(F.data == "img_auto_preen")
async def img_auto_preen_handler(callback: CallbackQuery, state: FSMContext):
...
@router.callback_query(F.data=="mavzu_preen",PresState__en.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Submit a presentation topic:")
    await state.update_data({"mm_id": a.message_id})

    await state.set_state(PresState__en.mavzu)
    await callback.answer()
    print(2)
@router.callback_query(F.data=="til_preen",PresState__en.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
    ...
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(PresState__en.til)
    await callback.answer()
    print(1)


@router.callback_query(F.data=="muallif_preen",PresState__en.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):

        a = await callback.message.answer("Submit author name:")
        await state.update_data({"mm_id": a.message_id})
        await state.set_state(PresState__en.muallif)
        await callback.answer()
        print(3)

@router.callback_query(F.data == "ozgartirish_preen", PresState__en.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ol=await state.get_data()
    m_id=ol.get("m_id")
    uid=callback.from_user.id
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Subject", callback_data="mavzu_preen"),
                InlineKeyboardButton(text="Author ", callback_data="muallif_preen"),
                InlineKeyboardButton(text="Language", callback_data="til_preen")
            ],
            [
                InlineKeyboardButton(text="-", callback_data="minus_preen"),
                InlineKeyboardButton(text=f"Pages: {requested_slide_count}", callback_data=f"count:{requested_slide_count}"),
                InlineKeyboardButton(text="+", callback_data="plus_preen")
            ],
            [
                InlineKeyboardButton(text="⬅️", callback_data="oldingi_preen"),
                InlineKeyboardButton(text=f"Design {bg_num}", callback_data=f"dizayn:{bg_num}"),
                InlineKeyboardButton(text="➡️", callback_data="keyingi_preen")
            ],
            [
                InlineKeyboardButton(text="Save ✅", callback_data="saqlash_preen"),
            ]
        ]
    )
    photo_manzili = Path(rf".\handlers\users\rasimlar\{bg_num}.jpg")
    rasim = FSInputFile(photo_manzili)
    a=await bot.send_photo(chat_id=callback.from_user.id,caption=
        "🌟Great, please check the following information.\n\n"
f"Topic: {topic}\n"
f"Author: {author}\n"
f"Number of pages: {requested_slide_count}\n"
f"Language: {lan}\n\n"
,
    ...
    await state.update_data({"m_id":a.message_id})
    await state.set_state(PresState__en.ozgartirish)




@router.callback_query(PresState__en.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):

    data=callback.data

    if data=='saqlash_preen':
        uid = callback.from_user.id
...
@router.callback_query(F.data == "rad_etish_preen", PresState__en.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    await bot.send_message(chat_id=callback.from_user.id,text="Presentation canceled.\n"
...
@router.callback_query(F.data == "tayyorlash_preen", PresState__en.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ol=await state.get_data()
    m_id=ol["m_id"]
    await bot.delete_message(chat_id=callback.from_user.id,message_id=m_id)
...
@router.callback_query(StateFilter(PresState__en.til))
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(text="-", callback_data="minus_preen"),
                InlineKeyboardButton(
                    text=f"Pages: {requested_slide_count}",
                    callback_data=f"count:{requested_slide_count}"
                ),
                InlineKeyboardButton(text="+", callback_data="plus_preen")
            ],
...
@router.message(StateFilter(PresState__en.muallif))
...
@router.message(StateFilter(PresState__en.mavzu))
async def handle_mavzu_message(message: Message, state: FSMContext):
...
@router.message(StateFilter(PresState__en.reja))
async def reja_input(msg: Message, state: FSMContext):
    if msg.text.lower() == "tayyor_preen":
        if len(user_data[msg.from_user.id].get("reja", [])) == 0:
            await msg.answer("❌ Include at least one plan item!")
            return
        await msg.answer("📄 How many slides (number only):")
        await state.set_state(PresState__en.slide_count)
    else:
