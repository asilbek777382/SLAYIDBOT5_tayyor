...
# --- FSM States ---
class ReferatState_ru_ru_ru(StatesGroup):
    doc_type = State()
    title = State()
    institute = State()
    author = State()
...
# --- Keyboard functions ---
def start_keyboard_ru():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📘 Referat / Mustaqil ish", callback_data="start_referat")]
    ])


def doc_type_keyboard_ru():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="РЕФЕРАТ ", callback_data="set_type_ru_referat")],
        [InlineKeyboardButton(text="САМОСТОЯТЕЛЬНАЯ РАБОТА", callback_data="set_type_ru_mustaqil")]
    ])
...
def language_keyboard_ru():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="О’zbek", callback_data="langru_uzbek"),
            InlineKeyboardButton(text="Русский", callback_data="langru_rus"),
            InlineKeyboardButton(text="English", callback_data="langru_english")
        ]
    ])
...
@router.callback_query(F.data == "referat_ru")
async def start_referat(callback: CallbackQuery, state: FSMContext):
    ...
    if olish:
        ...
        if int(summa[3]) >= tekshir:
            ...
            await state.set_state(RefesState_shablon_ru.title)
        ...
        if int(summa[3]) >= tekshir:
            ...
            if int(check[3]) < 2500:
                await callback.message.answer(
                    f"У вас недостаточно средств на счете 🙅!\Ваш счет:💰 {check[3]}\n\Вам нужно /bye, чтобы пополнить счет.")

            else:

                await state.set_state(ReferatState_ru_ru_ru.doc_type)
                photo_manzili = Path(r".\handlers\users\img_1.png")
                rasim = FSInputFile(photo_manzili)
                await bot.send_photo(chat_id=callback.from_user.id, caption="Выбирать:", photo=rasim,
                                     reply_markup=doc_type_keyboard_ru())
        else:
            text = (f"💰Ваш баланс: {pul[1]} сумов\n\n"
f"Используйте команду /bye, чтобы пополнить свой счёт.")
            await callback.message.answer(text, parse_mode="Markdown")

@router.callback_query(F.data.startswith("set_type_ru_"))
async def set_doc_type(callback: CallbackQuery, state: FSMContext):
    doc_type = "REFERAT" if callback.data == "set_type_ru_referat" else "MUSTAQIL ISH"
    await state.update_data(doc_type=doc_type)
    await callback.message.answer("Введите тему:")
    await state.set_state(ReferatState_ru_ru_ru.title)


@router.message(StateFilter(ReferatState_ru_ru_ru.title))
async def get_title(message: Message, state: FSMContext):
    await state.update_data(title=clean_text_ru(message.text))
    await message.answer("Введите институт и факультет:")
    await state.set_state(ReferatState_ru_ru_ru.institute)


@router.message(StateFilter(ReferatState_ru_ru_ru.institute))
async def get_institute(message: Message, state: FSMContext):
    ...
    await state.set_state(ReferatState_ru_ru_ru.author)


@router.message(StateFilter(ReferatState_ru_ru_ru.author))
async def get_author(message: Message, state: FSMContext):
    await state.update_data(author=clean_text_ru(message.text))
    await message.answer("Введите количество страниц основного текста (страницы 5-20):")
    await state.set_state(ReferatState_ru_ru_ru.pages)


@router.message(StateFilter(ReferatState_ru_ru_ru.pages))
async def get_pages(message: Message, state: FSMContext):

    if not message.text.isdigit():
        await message.answer("Пожалуйста, введите только число (5-20):1")
        return
    summa = db.select_summa()
    tek = int(message.text) * int(summa[1])
    pages = int(message.text)
    if not 5 <= pages <= 20:
        await message.answer("Введите страницы с 5 по 20.")
        return

    check=db.select_user(tg_user=message.from_user.id)
    if int(check[3]) < tek:
        await message.message.answer(
            f"У вас недостаточно средств на счете 🙅!\Ваш счет:💰 {check[3]}\n\n Нажмите /bye, чтобы пополнить счет.")
    else:
        ...
        a=await message.answer("На каком языке это должно быть написано?", reply_markup=language_keyboard_ru())
        await state.update_data(m_id=a.message_id)
        await state.set_state(ReferatState_ru_ru_ru.language)


@router.callback_query(F.data.startswith("langru_"))
...
f"• Нажмите кнопку «✏️ Изменить», чтобы внести изменения.\n"
f"• Нажмите кнопку «🚫 Отклонить», чтобы отменить.",
                               reply_markup=kb
                               )
    await state.update_data({"m_id": a.message_id})
    await state.set_state(ReferatState_ru_ru_ru.tanlov)


@router.callback_query(F.data == "ozgartirish", ReferatState_ru_ru_ru.tanlov)
...
f"Язык: {til}\n\n",
    ...
    await state.update_data({"m_id": a.message_id})
    await state.set_state(ReferatState_ru_ru_ru.ozgartirish)


@router.callback_query(F.data == "mavzu", ReferatState_ru_ru_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Отправить абстрактную тему:")
    await state.update_data({"mm_id": a.message_id})

    await state.set_state(ReferatState_ru_ru_ru.mavzu)
    await callback.answer()
    print(2)


@router.callback_query(F.data == "til", ReferatState_ru_ru_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    a = await callback.message.answer("Выберите язык презентации:", reply_markup=keyboard)
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_ru_ru_ru.til)
    await callback.answer()
    print(1)


@router.callback_query(F.data == "muallif", ReferatState_ru_ru_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Укажите имя автора:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_ru_ru_ru.muallif)
    await callback.answer()
    print(3)


@router.callback_query(F.data == "institut", ReferatState_ru_ru_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Введите институт и факультет:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_ru_ru_ru.institutt)
    await callback.answer()
    print(4)


@router.callback_query(ReferatState_ru_ru_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    data = callback.data
...
@router.callback_query(F.data == "rad_etish", ReferatState_ru_ru_ru.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
...
async def reja_manual(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(ReferatState_ru_ru_ru.reja_manual)


@router.callback_query(F.data == "tayyorlash", ReferatState_ru_ru_ru.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ol = await state.get_data()
    m_id = ol["m_id"]
    await bot.delete_message(chat_id=callback.from_user.id, message_id=m_id)
...
@router.callback_query(StateFilter(ReferatState_ru_ru_ru.til))
...
@router.message(StateFilter(ReferatState_ru_ru_ru.muallif))
...
@router.message(StateFilter(ReferatState_ru_ru_ru.mavzu))
async def handle_mavzu_message(message: Message, state: FSMContext):
...
@router.message(StateFilter(ReferatState_ru_ru_ru.institutt))
async def handle_mavzu_message(message: Message, state: FSMContext):
    ol = await state.get_data()
    mm_id = ol.get("mm_id")
    await bot.delete_message(chat_id=message.from_user.id, message_id=mm_id)
...
@router.message(StateFilter(ReferatState_ru_ru_ru.reja_manual))
async def get_manual_reja(message: Message, state: FSMContext):
    ...
    if len(reja) < 3:
        await message.answer("Пожалуйста, включите как минимум 3 пункта плана:")
        return
...
async def generate_and_send_referat_ru(message: Message, state: FSMContext):
    ...
    try:
        ...
        caption = (f"📌 {data['title']}\n"
                   ...
                   f"💡 @Slaydai_bot\n\n"
                   f"Нажмите /start, чтобы выполнить другое действие.")
