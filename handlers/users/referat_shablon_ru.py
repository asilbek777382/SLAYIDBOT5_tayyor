...
# --- FSM States ---
class RefesState_shablon_ru(StatesGroup):
    doc_type = State()
    title = State()
    institute = State()
    author = State()
...
@router.message(StateFilter(RefesState_shablon_ru.title))
...
f"• Нажмите кнопку «🚫 Отклонить», чтобы отменить.",
    ...
    await state.set_state(RefesState_shablon_ru.tanlov)


@router.callback_query(F.data == "ozgartirish", RefesState_shablon_ru.tanlov)
...
f"Язык: {til}\n\n",
    ...
    await state.update_data({"m_id": a.message_id})
    await state.set_state(RefesState_shablon_ru.ozgartirish)


@router.callback_query(F.data == "mavzu", RefesState_shablon_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Отправить абстрактную тему:")
    await state.update_data({"mm_id": a.message_id})

    await state.set_state(RefesState_shablon_ru.mavzu)
    await callback.answer()
    print(2)


@router.callback_query(F.data == "til", RefesState_shablon_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(RefesState_shablon_ru.til)
    await callback.answer()
    print(1)


@router.callback_query(F.data == "muallif", RefesState_shablon_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Укажите имя автора:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(RefesState_shablon_ru.muallif)
    await callback.answer()
    print(3)


@router.callback_query(F.data == "institut", RefesState_shablon_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Введите институт и факультет:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(RefesState_shablon_ru.institutt)
    await callback.answer()
    print(4)


@router.callback_query(RefesState_shablon_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    data = callback.data
...
@router.callback_query(F.data == "rad_etish", RefesState_shablon_ru.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
...
async def reja_manual(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(RefesState_shablon_ru.reja_manual)


@router.callback_query(F.data == "tayyorlash", RefesState_shablon_ru.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ol = await state.get_data()
    m_id = ol["m_id"]
    await bot.delete_message(chat_id=callback.from_user.id, message_id=m_id)
...
@router.callback_query(StateFilter(RefesState_shablon_ru.til))
...
@router.message(StateFilter(RefesState_shablon_ru.muallif))
...
@router.message(StateFilter(RefesState_shablon_ru.mavzu))
async def handle_mavzu_message(message: Message, state: FSMContext):
...
@router.message(StateFilter(RefesState_shablon_ru.institutt))
async def handle_mavzu_message(message: Message, state: FSMContext):
    ol = await state.get_data()
    mm_id = ol.get("mm_id")
    await bot.delete_message(chat_id=message.from_user.id, message_id=mm_id)
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(text="Автор", callback_data="til"),
                InlineKeyboardButton(text="Институт", callback_data="institut")
            ],
            [
                InlineKeyboardButton(text="-", callback_data="minus"),
