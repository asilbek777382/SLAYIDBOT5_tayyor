...
# --- FSM States ---
class RefesStatekurs_shablon_en(StatesGroup):
    doc_type = State()
    title = State()
    institute = State()
    author = State()
...
@router.message(StateFilter(RefesStatekurs_shablon_en.title))
async def get_title(message: Message, state: FSMContext):
    ...
    await state.update_data(doc_type='COURSE WORK')
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                ...
                InlineKeyboardButton(text="🚫 Deviation", callback_data="rad_etish"),
    ...
    f"• Click the button «✏️ Edit», to make changes.\n"
    f"• Click the button «🚫 Reject», cancel.",
                               reply_markup=kb
                               )
    await state.update_data({"m_id": a.message_id})
    await state.set_state(RefesStatekurs_shablon_en.tanlov)


@router.callback_query(F.data == "ozgartirish", RefesStatekurs_shablon_en.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Item", callback_data="mavzu"),
    ...
    await state.update_data({"m_id": a.message_id})
    await state.set_state(RefesStatekurs_shablon_en.ozgartirish)


@router.callback_query(F.data == "mavzu", RefesStatekurs_shablon_en.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Suggest a topic:")
    await state.update_data({"mm_id": a.message_id})

    await state.set_state(RefesStatekurs_shablon_en.mavzu)
    await callback.answer()
    print(2)


@router.callback_query(F.data == "til", RefesStatekurs_shablon_en.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    a = await callback.message.answer("Enter the institute and faculty:", reply_markup=keyboard)
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(RefesStatekurs_shablon_en.til)
    await callback.answer()
    print(1)


@router.callback_query(F.data == "muallif", RefesStatekurs_shablon_en.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Please provide the author's name:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(RefesStatekurs_shablon_en.muallif)
    await callback.answer()
    print(3)


@router.callback_query(F.data == "institut", RefesStatekurs_shablon_en.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Enter the institute and faculty:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(RefesStatekurs_shablon_en.institutt)
    await callback.answer()
    print(4)


@router.callback_query(RefesStatekurs_shablon_en.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    data = callback.data
...
@router.callback_query(F.data == "rad_etish", RefesStatekurs_shablon_en.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    await bot.send_message(chat_id=callback.from_user.id, text="Presentation cancelled.\n"
...
async def reja_manual(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(RefesStatekurs_shablon_en.reja_manual)


@router.callback_query(F.data == "tayyorlash", RefesStatekurs_shablon_en.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ol = await state.get_data()
    m_id = ol["m_id"]
    await bot.delete_message(chat_id=callback.from_user.id, message_id=m_id)
...
@router.callback_query(StateFilter(RefesStatekurs_shablon_en.til))
...
@router.message(StateFilter(RefesStatekurs_shablon_en.muallif))
...
@router.message(StateFilter(RefesStatekurs_shablon_en.mavzu))
async def handle_mavzu_message(message: Message, state: FSMContext):
...
@router.message(StateFilter(RefesStatekurs_shablon_en.institutt))
async def handle_mavzu_message(message: Message, state: FSMContext):
    ol = await state.get_data()
    mm_id = ol.get("mm_id")
    await bot.delete_message(chat_id=message.from_user.id, message_id=mm_id)
...
async def re_generate_and_send_referat(message: Message, state: FSMContext):
    ...
    try:
        ...
        caption = (f"📌 {data['title']}\n"
                   ...
                   f"💡 @Slaydai_bot\n\n"
                   f"/start Press and hold to perform another action..")
