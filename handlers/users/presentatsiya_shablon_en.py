...
# -------- STATES --------
class PresState_shablon__en(StatesGroup):
    language = State()
    topic = State()
    author = State()
    bg = State()
...
class ImageCollectState_shablon__en(StatesGroup):
    collecting = State()
...
@router.message(StateFilter(PresState_shablon__en.topic))
async def get_topic(msg: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Image selection", callback_data="img_select_shab__en")],
        [InlineKeyboardButton(text="♻️ Automatic pictures", callback_data="img_auto_shab__en")],
        [InlineKeyboardButton(text="🚫 Without picture", callback_data="img_none_shab__en")]
    ])
...
@router.callback_query(F.data == "img_select_shab__en")
...
"🌃 Photos: 0/7",
    ...
    await state.set_state(ImageCollectState_shablon__en.collecting)

@router.message(StateFilter(ImageCollectState_shablon__en.collecting), F.photo)
async def collect_images(msg: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Finish", callback_data="img_finish_preen_shab")],
        [InlineKeyboardButton(text="♻ Delete pictures", callback_data="img_reset_preen_shab")]
    ])
    await msg.answer(f"🌃 Pictures: {len(user_images[uid])}/7", reply_markup=kb)

@router.callback_query(F.data == "img_reset_preen_shab")
...
@router.callback_query(F.data == "img_finish_preen_shab")
async def finish_images(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="✅ Preparation", callback_data="tayyorlash_shab_preen"),
                InlineKeyboardButton(text="🚫 Rejection", callback_data="rad_etish_shab_preen"),
                InlineKeyboardButton(text="✏️ Change", callback_data="ozgartirish_shab_preen")
            ]
        ]
    )
...
f"• Click '🚫 Reject' to cancel",
    ...
    await state.update_data({"m_id": a.message_id})
    await state.set_state(PresState_shablon__en.tanlov)
    await callback.answer()

# --- Rasmsiz rejim ---
@router.callback_query(F.data == "img_none_shab__en")
...
@router.callback_query(F.data == "img_auto_shab__en")
...
@router.callback_query(F.data == "ozgartirish_shab_preen", PresState_shablon__en.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Subject", callback_data="mavzu_shab_preen"),
                InlineKeyboardButton(text="Author ", callback_data="muallif_shab_preen"),
                InlineKeyboardButton(text="Language", callback_data="til_shab_preen")
            ],
            [
                InlineKeyboardButton(text="-", callback_data="minus_shab_preen"),
                InlineKeyboardButton(text=f"Pages: {requested_slide_count}", callback_data=f"count:{requested_slide_count}"),
                InlineKeyboardButton(text="+", callback_data="plus_shab_preen")
            ],
            [
                InlineKeyboardButton(text="⬅️", callback_data="oldingi_shab_preen"),
                InlineKeyboardButton(text=f"Dizayn {bg_num}", callback_data=f"dizayn:{bg_num}"),
                InlineKeyboardButton(text="➡️", callback_data="keyingi_shab_preen")
            ],
            [
                InlineKeyboardButton(text="Save ✅", callback_data="saqlash_shab_preen"),
            ]
        ]
    )
    photo_manzili = Path(rf".\handlers\users\rasimlar\{bg_num}.jpg")
...
f"• Click '🚫 Reject' to cancel",
    ...
    await state.update_data({"m_id":a.message_id})
    await state.set_state(PresState_shablon__en.ozgartirish)



@router.callback_query(F.data=="mavzu_shab_preen",PresState_shablon__en.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Submit a presentation topic:")
    await state.update_data({"mm_id": a.message_id})

    await state.set_state(PresState_shablon__en.mavzu)
    await callback.answer()
    print(2)
@router.callback_query(F.data=="til_shab_preen",PresState_shablon__en.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(PresState_shablon__en.til)
    await callback.answer()
    print(1)


@router.callback_query(F.data=="muallif_shab_preen",PresState_shablon__en.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):

        a = await callback.message.answer("Submit author name:")
        await state.update_data({"mm_id": a.message_id})
        await state.set_state(PresState_shablon__en.muallif)
        await callback.answer()
        print(3)


@router.callback_query(PresState_shablon__en.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    """
    ...
    if data == 'saqlash_shab_preen':
        ...
        elif data == "minus_shab_preen":
            if int(requested_slide_count) < 6:
                await callback.answer("You can select at least 5 pages.❗️", show_alert=True)
                await state.set_state(PresState_shablon__en.ozgartirish)
            else:
                user_data.setdefault(uid, {})["requested_slide_count"] = int(requested_slide_count) - 1
                await state.set_state(PresState_shablon__en.ozgartirish)
...
"• If the information is correct, click '✅ Prepare'!\n"
"• Use the buttons above to change"
            )
        )
...
@router.callback_query(F.data == "rad_etish_shab_preen", PresState_shablon__en.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
...
@router.callback_query(StateFilter(PresState_shablon__en.til))
async def handle_mavzu_message(message: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(text="-", callback_data="minus_shab_preen"),
                InlineKeyboardButton(text=f"Language: {requested_slide_count}",
                                     callback_data=f"count:{requested_slide_count}"),
                InlineKeyboardButton(text="+", callback_data="plus_shab_preen")
            ],
            [
...
@router.message(StateFilter(PresState_shablon__en.muallif))
...
@router.message(StateFilter(PresState_shablon__en.mavzu))
async def handle_mavzu_message(message: Message, state: FSMContext):
...
@router.callback_query(F.data == "tayyorlash_shab_preen", PresState_shablon__en.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ol=await state.get_data()
    m_id=ol["m_id"]
    await bot.delete_message(chat_id=callback.from_user.id,message_id=m_id)
