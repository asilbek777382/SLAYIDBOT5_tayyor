...
from handlers.users.kurs_ishi_shablon_en import RefesStatekurs_shablon_en
...
# --- FSM States ---
class ReferatState_uz_uz_kurs(StatesGroup):
    doc_type = State()
    title = State()
    institute = State()
    author = State()
...
def language_keyboard_ru():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="О’zbek", callback_data="languzkurs_uzbek"),
            InlineKeyboardButton(text="Русский", callback_data="languzkurs_rus"),
            InlineKeyboardButton(text="English", callback_data="languzkurs_english")
        ]
    ])
def sahifalar_soni():
    return InlineKeyboardMarkup(inline_keyboard=[
...
async def start_referat(callback: CallbackQuery, state: FSMContext):
    ...
    if olish:
        ...
        if int(summa[3]) >= tekshir:
            ...
            await state.set_state(RefesStatekurs_shablon_uz.title)
        ...
        if int(summa[3]) >= tekshir:
            ...
            if int(check[3]) < 2500:
                await callback.message.answer(
                    f"Hisobingizda yetarli mablag‘ yo‘q 🙅!\nHisobingiz:💰 {check[3]}\n\Hisobingizni to‘ldirish uchun /bye tugmasini bosing.")

            else:
                await state.update_data(doc_type='kurs_ishi')
                await callback.message.answer("Mavzuni kiriting:")
                await state.set_state(ReferatState_uz_uz_kurs.title)

        else:
            text = (f"💰Balansingiz: {pul[1]} soʻm\n\n"
f"Hisobingizni to'ldirish uchun /bye buyrug'idan foydalaning.")
            await callback.message.answer(text, parse_mode="Markdown")



@router.message(StateFilter(ReferatState_uz_uz_kurs.title))
async def get_title(message: Message, state: FSMContext):
    await state.update_data(title=clean_text_ru(message.text))
    await message.answer("Institut va fakultetga kiring:")
    await state.set_state(ReferatState_uz_uz_kurs.institute)


@router.message(StateFilter(ReferatState_uz_uz_kurs.institute))
async def get_institute(message: Message, state: FSMContext):
    await state.update_data(institute=clean_text_ru(message.text))
    await message.answer("Muallifni kiriting (Ismi Familiya):")
    await state.set_state(ReferatState_uz_uz_kurs.author)


@router.message(StateFilter(ReferatState_uz_uz_kurs.author))
async def get_author(message: Message, state: FSMContext):
    await state.update_data(author=clean_text_ru(message.text))
    await message.answer("Asosiy matnning sahifalar sonini tanlang",reply_markup=sahifalar_soni())
    await state.set_state(ReferatState_uz_uz_kurs.pages)


@router.callback_query(ReferatState_uz_uz_kurs.pages)
async def get_pages(message: CallbackQuery, state: FSMContext):

    sahifa_soni=message.data.rsplit('-')
    await state.update_data(sahifa_soni=sahifa_soni)
    ...
    if int(check[3]) < tek:
        await bot.send_message(chat_id=message.from_user.id,text=
            f"Hisobingizda yetarli mablag‘ yo‘q 🙅!\Hisobingiz:💰 {check[3]}\n\n Hisobingizni to‘ldirish uchun /bye tugmasini bosing.",reply_markup=sahifalar_soni)
    else:


        a=await message.message.answer("Bu qaysi tilda yozilishi kerak?", reply_markup=language_keyboard_ru())
        await state.update_data(m_id=a.message_id)
        await state.set_state(ReferatState_uz_uz_kurs.language)


@router.callback_query(F.data.startswith("languzkurs_"))
async def get_language(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="✅ Tayyorgarlik", callback_data="tayyorlash"),
...
f"Til: {til}\n\n"
f"• Agar ma'lumotlar to'g'ri bo'lsa, tugmani bosing «✅ Tayyorlang»!\n"
f"• Tugmani bosing «✏️ O'zgartirish», o'zgartirishlar kiritish.\n"
f"• Tugmani bosing «🚫 Rad etish», bekor qilish.",
                               reply_markup=kb
                               )
    await state.update_data({"m_id": a.message_id})
    await state.set_state(ReferatState_uz_uz_kurs.tanlov)


@router.callback_query(F.data == "ozgartirish", ReferatState_uz_uz_kurs.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Fan", callback_data="mavzu"),
...
f"Til: {til}\n\n",
    ...
    await state.update_data({"m_id": a.message_id})
    await state.set_state(ReferatState_uz_uz_kurs.ozgartirish)


@router.callback_query(F.data == "mavzu", ReferatState_uz_uz_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Mavzuni yuboring:")
    await state.update_data({"mm_id": a.message_id})

    await state.set_state(ReferatState_uz_uz_kurs.mavzu)
    await callback.answer()
    print(2)


@router.callback_query(F.data == "til", ReferatState_uz_uz_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_uz_uz_kurs.til)
    await callback.answer()
    print(1)


@router.callback_query(F.data == "muallif", ReferatState_uz_uz_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Iltimos, muallifning ismini ko'rsating:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_uz_uz_kurs.muallif)
    await callback.answer()
    print(3)


@router.callback_query(F.data == "institut", ReferatState_uz_uz_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Institut va fakultetga kiring:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_uz_uz_kurs.institutt)
    await callback.answer()
    print(4)


@router.callback_query(ReferatState_uz_uz_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    data = callback.data
    ...
    if data == 'saqlash':
        ...
        if data == "plus":
            if int(requested_slide_count) > 104:
                await callback.answer("Siz maksimal 20 sahifani tanlashingiz mumkin.❗️", show_alert=True)
            else:
        ...
        if data == "minus":
            if int(requested_slide_count) < 25:
                await callback.answer("Siz maksimal 5 sahifani tanlashingiz mumkin.❗️", show_alert=True)
            else:
                await state.update_data({"pages": int(
                    requested_slide_count) - 5})
...
@router.callback_query(F.data == "rad_etish", ReferatState_uz_uz_kurs.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
...
async def reja_manual(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Reja bandlarini kiriting (har birini yangi qatorga yozing):")
    await state.set_state(ReferatState_uz_uz_kurs.reja_manual)


@router.callback_query(F.data == "tayyorlash", ReferatState_uz_uz_kurs.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ol = await state.get_data()
    m_id = ol["m_id"]
    await bot.delete_message(chat_id=callback.from_user.id, message_id=m_id)
...
@router.callback_query(StateFilter(ReferatState_uz_uz_kurs.til))
...
@router.message(StateFilter(ReferatState_uz_uz_kurs.muallif))
...
@router.message(StateFilter(ReferatState_uz_uz_kurs.mavzu))
async def handle_mavzu_message(message: Message, state: FSMContext):
...
@router.message(StateFilter(ReferatState_uz_uz_kurs.institutt))
async def handle_mavzu_message(message: Message, state: FSMContext):
    ol = await state.get_data()
    mm_id = ol.get("mm_id")
    await bot.delete_message(chat_id=message.from_user.id, message_id=mm_id)
...
@router.message(StateFilter(ReferatState_uz_uz_kurs.reja_manual))
...
async def generate_and_send_referat_ru(message: Message, state: FSMContext):
    ...
    db.add_shablon_kurs(tg_id=from_id, institut=ins, ism_fam=ism, sahifa_soni=requested_slide_count,
                        til='uz', kurs_tili=til)
    progress_msg = await message.answer("Annotatsiya tayyorlanmoqda...")
    #
    ol = await state.get_data()
    ...
    try:
        ...
        for step in range(1, total_steps + 1):
            ...
            await progress_msg.edit_text(f"🕒 Tayyorgarlik:\n{bar}")
        ...
        caption = (f"📌 {data['title']}\n"
                   ...
                   f"💡 @Slaydai_bot\n\n"
                   f"Boshqa amalni bajarish uchun /start tugmasini bosing.")
...
async def send_short_version(message: Message, state: FSMContext):
    ...
    progress_msg = await message.answer("Готовится сокращенная версия....")
