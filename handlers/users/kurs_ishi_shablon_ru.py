...
# --- FSM States ---
class RefesStatekurs_shablon_ru(StatesGroup):
    doc_type = State()
    title = State()
    institute = State()
    author = State()
...
def language_keyboard_en():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="О’zbek", callback_data="langenn_uzbek"),
            InlineKeyboardButton(text="Русский", callback_data="langenn_rus"),
            InlineKeyboardButton(text="English", callback_data="langenn_english")
        ]
    ])
...
async def get_title(message: Message, state: FSMContext):
    ...
    f"• Нажмите кнопку «✏️ Редактировать», чтобы внести изменения.\n"
    f"• Нажмите кнопку «🚫 Отклонить», отмените.",
                               reply_markup=kb
                               )
    await state.update_data({"m_id": a.message_id})
    await state.set_state(RefesStatekurs_shablon_ru.tanlov)


@router.callback_query(F.data == "ozgartirish", RefesStatekurs_shablon_ru.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data({"m_id": a.message_id})
    await state.set_state(RefesStatekurs_shablon_ru.ozgartirish)


@router.callback_query(F.data == "mavzu", RefesStatekurs_shablon_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Предложить тему:")
    await state.update_data({"mm_id": a.message_id})

    await state.set_state(RefesStatekurs_shablon_ru.mavzu)
    await callback.answer()
    print(2)


@router.callback_query(F.data == "til", RefesStatekurs_shablon_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(RefesStatekurs_shablon_ru.til)
    await callback.answer()
    print(1)


@router.callback_query(F.data == "muallif", RefesStatekurs_shablon_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Пожалуйста, укажите имя автора:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(RefesStatekurs_shablon_ru.muallif)
    await callback.answer()
    print(3)


@router.callback_query(F.data == "institut", RefesStatekurs_shablon_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Введите институт и факультет:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(RefesStatekurs_shablon_ru.institutt)
    await callback.answer()
    print(4)


@router.callback_query(RefesStatekurs_shablon_ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    data = callback.data
    ...
    if data == 'saqlash':
        ...
        "🌟Отлично, ознакомьтесь с информацией ниже.\n\n"
        f"{doc_type}\n"
        f"Предмет: {mavzu}\n"
        f"Автор: {ism}\n"
        f"Институт и кафедра: {ins}\n"
...
@router.callback_query(F.data == "rad_etish", RefesStatekurs_shablon_ru.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
...
async def reja_manual(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(RefesStatekurs_shablon_ru.reja_manual)


@router.callback_query(F.data == "tayyorlash", RefesStatekurs_shablon_ru.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ol = await state.get_data()
    m_id = ol["m_id"]
    await bot.delete_message(chat_id=callback.from_user.id, message_id=m_id)
...
@router.callback_query(StateFilter(RefesStatekurs_shablon_ru.til))
...
@router.message(StateFilter(RefesStatekurs_shablon_ru.muallif))
...
@router.message(StateFilter(RefesStatekurs_shablon_ru.mavzu))
async def handle_mavzu_message(message: Message, state: FSMContext):
...
@router.message(StateFilter(RefesStatekurs_shablon_ru.institutt))
async def handle_mavzu_message(message: Message, state: FSMContext):
    ol = await state.get_data()
    mm_id = ol.get("mm_id")
    await bot.delete_message(chat_id=message.from_user.id, message_id=mm_id)
...
async def re_generate_and_send_referat(message: Message, state: FSMContext):
    ...
    progress_msg = await message.answer("Реферат готовится...")
    #
    ol = await state.get_data()
    ...
    try:
        ...
        caption = (f"📌 {data['title']}\n"
                   ...
                   f"💡 @Slaydai_bot\n\n"
                   f"/start Нажмите и удерживайте, чтобы выполнить другое действие..")
