...
from handlers.users.kurs_ishi_shablon import RefesStatekurs_shablon_uz
from handlers.users.kurs_ishi_shablon_ru import RefesStatekurs_shablon_ru
from handlers.users.referat_shablon_ru import RefesState_shablon_ru
from handlers.users.referat_shablon_uz import RefesState_shablon_uz
...
# --- FSM States ---
class ReferatState_ru_kurs(StatesGroup):
    doc_type = State()
    title = State()
    institute = State()
    author = State()
...
def language_keyboard_ru():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="О’zbek", callback_data="langrukurs_uzbek"),
            InlineKeyboardButton(text="Русский", callback_data="langrukurs_rus"),
            InlineKeyboardButton(text="English", callback_data="langrukurs_english")
        ]
    ])
def sahifalar_soni():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="30-40", callback_data="30-40"),
            InlineKeyboardButton(text="50-60", callback_data="50-60"),
        ],
        [
            InlineKeyboardButton(text="70-80", callback_data="70-80"),
            InlineKeyboardButton(text="80-90", callback_data="80-90"),
            InlineKeyboardButton(text="90-100", callback_data="90-100"),
        ],

    ])

# --- AI communication function ---
async def ai_post_ru(payload):
...
async def generate_reja_ru(title: str, lang: str = 'uzbek') -> list:
    lang_prompts = {
    ...
    try:
        response = await ai_post_ru(payload)
...
async def generate_coursework_ru(title: str, reja: list, pages: int, lang: str = 'uzbek') -> str:
...
# --- Estimate page count ---
def estimate_page_count_ru(text: str) -> int:
...
def add_double_border_ru(section):
...
def save_to_word_ru(title, institute, author, pages, language, text, reja, doc_type):
    doc = Document()
    ...
    for para in paragraphs:
        ...
        # Oddiy matnni qayta ishlash
            ...
            # Formatni tozalash
            cleaned_para = clean_text_ru(para)
...
async def generate_targeted_content_ru(title: str, reja: list, target_pages: int, lang: str) -> str:
    """
    ...
    for i, item in enumerate(reja):
        ...
        try:
            ...
            cleaned_content = clean_text_ru(content)
            ...
            if actual_words < section_words * 0.8:  # 80% dan kam bo'lsa
                ...
                additional_response = await ai_post_ru(additional_payload)
                additional_content = additional_response.json().get("choices", [{}])[0].get("message", {}).get(
                    "content", "")
                cleaned_content += " " + clean_text_ru(additional_content)
...
@router.callback_query(F.data == "kurs_ishi_ru")
async def start_referat(callback: CallbackQuery, state: FSMContext):
    olish = db.select_shablon_kurs(tg_id=callback.from_user.id, til='ru')
    ...
    if olish:
        ...
        if int(summa[3]) >= tekshir:
            ...
            await state.set_state(RefesStatekurs_shablon_ru.title)
        ...
        if int(summa[3]) >= tekshir:
            ...
            if int(check[3]) < 2500:
                await callback.message.answer(
                    f"На вашем счете недостаточно средств 🙅!\Ваш счет:💰 {check[3]}\n\nВам необходимо попрощаться, чтобы пополнить свой счет.")

            else:
                await state.update_data(doc_type='kurs_ishi')
                await callback.message.answer("Enter a subject:")
                await state.set_state(ReferatState_ru_kurs.title)

        else:
            text = (f"💰Ваш баланс: {pul[1]} сумма\n\n"
f"Используйте команду /bye для пополнения счёта.")
            await callback.message.answer(text, parse_mode="Markdown")



@router.message(StateFilter(ReferatState_ru_kurs.title))
async def get_title(message: Message, state: FSMContext):
    await state.update_data(title=clean_text_ru(message.text))
    await message.answer("Поступить в институт и на факультет:")
    await state.set_state(ReferatState_ru_kurs.institute)


@router.message(StateFilter(ReferatState_ru_kurs.institute))
async def get_institute(message: Message, state: FSMContext):
    await state.update_data(institute=clean_text_ru(message.text))
    await message.answer("Введите автора (Имя Фамилия):")
    await state.set_state(ReferatState_ru_kurs.author)


@router.message(StateFilter(ReferatState_ru_kurs.author))
async def get_author(message: Message, state: FSMContext):
    await state.update_data(author=clean_text_ru(message.text))
    await message.answer("Выберите количество страниц основного текста.",reply_markup=sahifalar_soni())
    await state.set_state(ReferatState_ru_kurs.pages)


@router.callback_query(ReferatState_ru_kurs.pages)
async def get_pages(message: CallbackQuery, state: FSMContext):

    sahifa_soni=message.data.rsplit('-')
    await state.update_data(sahifa_sonii=sahifa_soni)
    summa = db.select_summa()
    tek = int(sahifa_soni[0])+5 * int(summa[1])
    pages = int(sahifa_soni[0])+5


    check=db.select_user(tg_user=message.from_user.id)
    if int(check[3]) < tek:
        await bot.send_message(chat_id=message.from_user.id,text=
            f"На вашем счете недостаточно средств 🙅!\Ваш счет:💰 {check[3]}\n\nНажмите кнопку /bye, чтобы пополнить свой счет.",reply_markup=sahifalar_soni)
    else:

        await state.update_data(pages=pages)
        a=await message.message.answer("На каком языке это должно быть написано?", reply_markup=language_keyboard_ru())
        await state.update_data(m_id=a.message_id)
        await state.set_state(ReferatState_ru_kurs.language)


@router.callback_query(F.data.startswith("langrukurs_"))
async def get_language(callback: CallbackQuery, state: FSMContext):
    ...
    reja = await generate_reja_ru(data['title'], data['language'])
    await state.update_data(reja_items=reja, reja_type="ai")
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="✅ Подготовка", callback_data="tayyorlash"),
                InlineKeyboardButton(text="🚫 Отклонение", callback_data="rad_etish"),
                InlineKeyboardButton(text="✏️ Изменять", callback_data="ozgartirish")
            ]
        ]
    )
    ...
    f"Предмет: {mavzu}\n"
    f"Автор: {ism}\n"
    f"Институт и кафедра: {ins}\n"

    f"Количество страниц: {requested_slide_count}\n"
    f"Язык: {til}\n\n"
    f"• Если информация верна, нажмите кнопку «✅ Подготовить»!\n"
    f"• Нажмите кнопку «✏️ Изменить», чтобы изменить\n"
    f"• Нажмите кнопку «🚫 Отклонить», чтобы отменить.",
                               reply_markup=kb
                               )
    await state.update_data({"m_id": a.message_id})
    await state.set_state(ReferatState_ru_kurs.tanlov)


@router.callback_query(F.data == "ozgartirish", ReferatState_ru_kurs.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Предмет", callback_data="mavzu"),
                InlineKeyboardButton(text="Автор ", callback_data="muallif")

            ],
            [
                InlineKeyboardButton(text="Язык", callback_data="til"),
                InlineKeyboardButton(text="Институт", callback_data="institut")
            ],
            [
            ...
            [
                InlineKeyboardButton(text="Сохранять ✅", callback_data="saqlash"),
            ]
        ]
    )
    a = await bot.send_message(chat_id=callback.from_user.id, text=
    ...
    await state.update_data({"m_id": a.message_id})
    await state.set_state(ReferatState_ru_kurs.ozgartirish)


@router.callback_query(F.data == "mavzu", ReferatState_ru_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Укажите имя автора:")
    await state.update_data({"mm_id": a.message_id})

    await state.set_state(ReferatState_ru_kurs.mavzu)
    await callback.answer()
    print(2)


@router.callback_query(F.data == "til", ReferatState_ru_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    a = await callback.message.answer("Выберите язык:", reply_markup=keyboard)
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_ru_kurs.til)
    await callback.answer()
    print(1)


@router.callback_query(F.data == "muallif", ReferatState_ru_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Укажите имя автора:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_ru_kurs.muallif)
    await callback.answer()
    print(3)


@router.callback_query(F.data == "institut", ReferatState_ru_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Введите институт и кафедру:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_ru_kurs.institutt)
    await callback.answer()
    print(4)


@router.callback_query(ReferatState_ru_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    data = callback.data
    ...
    if data == 'saqlash':
        ...
        await generate_and_send_referat_ru(callback.message, state)
        ...
        if data == "plus":
            if int(requested_slide_count) > 104:
        ...
        if data == "minus":
            if int(requested_slide_count) < 25:
...
@router.callback_query(F.data == "rad_etish", ReferatState_ru_kurs.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
...
async def reja_manual(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Введите пункты плана (пишите каждый с новой строки):")
    await state.set_state(ReferatState_ru_kurs.reja_manual)


@router.callback_query(F.data == "tayyorlash", ReferatState_ru_kurs.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ol = await state.get_data()
    m_id = ol["m_id"]
    await bot.delete_message(chat_id=callback.from_user.id, message_id=m_id)
...
@router.callback_query(StateFilter(ReferatState_ru_kurs.til))
async def handle_mavzu_message(message: CallbackQuery, state: FSMContext):
    til = message.data
    ol = await state.get_data()
    mm_id = ol.get("mm_id")
    db.update_shablon_lang(tg_id=message.from_user.id, pre_tili='ru', til=message.data)
...
@router.message(StateFilter(ReferatState_ru_kurs.muallif))
...
@router.message(StateFilter(ReferatState_ru_kurs.mavzu))
async def handle_mavzu_message(message: Message, state: FSMContext):
...
@router.message(StateFilter(ReferatState_ru_kurs.institutt))
async def handle_mavzu_message(message: Message, state: FSMContext):
    ol = await state.get_data()
    mm_id = ol.get("mm_id")
    await bot.delete_message(chat_id=message.from_user.id, message_id=mm_id)
...
@router.message(StateFilter(ReferatState_ru_kurs.reja_manual))
async def get_manual_reja(message: Message, state: FSMContext):
    reja = [f"{i + 1}. {clean_text_ru(line)}" for i, line in enumerate(message.text.split("\n")) if clean_text_ru(line)]
    if len(reja) < 3:
        await message.answer("Please enter at least 3 plan items:")
        return
    await state.update_data(reja_items=reja)
    await generate_and_send_referat_ru(message, state)


def get_progress_bar_ru(step: int, total_steps: int = 10) -> str:
    filled = "●" * step
...
async def generate_and_send_referat_ru(message: Message, state: FSMContext):
    data = await state.get_data()
    ...
    db.add_shablon_kurs(tg_id=from_id, institut=ins, ism_fam=ism, sahifa_soni=requested_slide_count,
                        til='ru', kurs_tili=til)
    progress_msg = await message.answer("Abstract is being prepared....")
    #
    ol = await state.get_data()
    sahifa_sonii = ol.get('sahifa_soni')
    user_balans = db.select_user(tg_user=message.from_user.id)
    summa = db.select_summa()
    puli = int(sahifa_sonii[0]) + 5 * int(summa[1])
    update = int(user_balans[3]) - int(puli)
    db.update_user_balans(tg_user=message.from_user.id, balans=update)

    pul = db.select_summa()
    summa = db.select_user(tg_user=message.from_user.id)
    tekshir = int(pul[1]) * 5
    if int(summa[3]) >= tekshir:
        check = db.select_user(tg_user=message.from_user.id)
        if int(check[3]) < 2500:
            await message.message.answer(
                f"На вашем счете недостаточно средств 🙅!\nВаш счет:💰 {check[3]}\nНажмите кнопку /bye, чтобы пополнить свой счет.")
            await state.clear()
    #
    try:
        total_steps = 15
        for step in range(1, total_steps + 1):
            bar = get_progress_bar_ru(step, total_steps)
            await progress_msg.edit_text(f"🕒 Preparation:\n{bar}")
            await asyncio.sleep(8)  # sekinroq progress

        # Maqsadli matn yaratish
        text = await generate_targeted_content_ru(data['title'], reja, requested_pages, lang)

        # Sahifalar sonini tekshirish
        actual_pages = estimate_page_count_ru(text)
        ...
        if page_difference > 2:  # 2 sahifadan ortiq farq bo'lsa
            ...
            if actual_pages > requested_pages:
                ...
                text = await generate_targeted_content_ru(data['title'], reja, adjusted_pages, lang)
            else:
                # Ko'paytirish kerak
                additional_pages = requested_pages - actual_pages
                additional_text = await generate_targeted_content_ru(data['title'], reja, additional_pages, lang)
                text += additional_text

        # Hujjatni saqlash
        file_path = save_to_word_ru(
        ...
        final_pages = estimate_page_count_ru(text)
        caption = (f"📌 {data['title']}\n"

                   f"💡 @Slaydai_bot\n\n"
                   f"Чтобы выполнить другое действие, нажмите кнопку /start.")
...
async def send_short_version(message: Message, state: FSMContext):
    ...
    if not long_text:
        await message.answer("Текст для сокращения не найден.")
        return

    progress_msg = await message.answer("Сокращенная версия готовится...")

    try:
        # Generate shorter version
        requested_pages = data['pages']
        text = await generate_coursework_ru(data['title'], data['reja_items'], requested_pages, data['language'])
        ...
        actual_pages = estimate_page_count_ru(text)
        await progress_msg.delete()
        caption = f"📌 {data['title']} (shortened)\n page\n💡 @Slaydai_bot\n\n Нажмите /start, чтобы выполнить другое действие."
        await message.answer_document(FSInputFile(file_path), caption=caption)
        os.remove(file_path)
