...
# --- FSM States ---
class RefesStatekurs_shablon_uz(StatesGroup):
    doc_type = State()
    title = State()
    institute = State()
    author = State()
...
async def get_title(message: Message, state: FSMContext):
    ...
    await state.set_state(RefesStatekurs_shablon_uz.tanlov)


@router.callback_query(F.data == "ozgartirish", RefesStatekurs_shablon_uz.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data({"m_id": a.message_id})
    await state.set_state(RefesStatekurs_shablon_uz.ozgartirish)


@router.callback_query(F.data == "mavzu", RefesStatekurs_shablon_uz.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Mavzuni yuboring:")
    await state.update_data({"mm_id": a.message_id})

    await state.set_state(RefesStatekurs_shablon_uz.mavzu)
    await callback.answer()
    print(2)


@router.callback_query(F.data == "til", RefesStatekurs_shablon_uz.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(RefesStatekurs_shablon_uz.til)
    await callback.answer()
    print(1)


@router.callback_query(F.data == "muallif", RefesStatekurs_shablon_uz.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Iltimos, muallifning ismini ko'rsating:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(RefesStatekurs_shablon_uz.muallif)
    await callback.answer()
    print(3)


@router.callback_query(F.data == "institut", RefesStatekurs_shablon_uz.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Institut va fakultetga kiring:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(RefesStatekurs_shablon_uz.institutt)
    await callback.answer()
    print(4)


@router.callback_query(RefesStatekurs_shablon_uz.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    data = callback.data
...
@router.callback_query(F.data == "rad_etish", RefesStatekurs_shablon_uz.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
...
async def reja_manual(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(RefesStatekurs_shablon_uz.reja_manual)


@router.callback_query(F.data == "tayyorlash", RefesStatekurs_shablon_uz.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ol = await state.get_data()
    m_id = ol["m_id"]
    await bot.delete_message(chat_id=callback.from_user.id, message_id=m_id)
...
@router.callback_query(StateFilter(RefesStatekurs_shablon_uz.til))
...
@router.message(StateFilter(RefesStatekurs_shablon_uz.muallif))
...
@router.message(StateFilter(RefesStatekurs_shablon_uz.mavzu))
async def handle_mavzu_message(message: Message, state: FSMContext):
...
@router.message(StateFilter(RefesStatekurs_shablon_uz.institutt))
async def handle_mavzu_message(message: Message, state: FSMContext):
    ol = await state.get_data()
    mm_id = ol.get("mm_id")
    await bot.delete_message(chat_id=message.from_user.id, message_id=mm_id)
...
async def re_generate_and_send_referat(message: Message, state: FSMContext):
    ...
    try:
        ...
        caption = (f"📌 {data['title']}\n"
                   ...
                   f"💡 @Slaydai_bot\n\n"
                   f"/start Boshqa amalni bajarish uchun bosing va ushlab turing.")
