...
# --- FSM States ---
class ReferatState_en_kurs(StatesGroup):
    doc_type = State()
    title = State()
    institute = State()
    author = State()
...
def language_keyboard_ru():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="О’zbek", callback_data="langenkurs_uzbek"),
            InlineKeyboardButton(text="Русский", callback_data="langenkurs_rus"),
            InlineKeyboardButton(text="English", callback_data="langenkurs_english")
        ]
    ])
def sahifalar_soni():
    return InlineKeyboardMarkup(inline_keyboard=[
...
@router.callback_query(F.data == "kurs_ishi_en")
async def start_referat(callback: CallbackQuery, state: FSMContext):
    ...
    if olish:
        ...
        if int(summa[3]) >= tekshir:
            ...
            await state.set_state(RefesStatekurs_shablon_en.title)
        ...
        if int(summa[3]) >= tekshir:
            ...
            if int(check[3]) < 2500:
                await callback.message.answer(
                    f"There are not enough funds in your account 🙅!\Your account:💰 {check[3]}\n\You need to /say goodbye to top up your account.")

            else:
                await state.update_data(doc_type='kurs_ishi')
                await callback.message.answer("Enter a subject:")
                await state.set_state(ReferatState_en_kurs.title)

        else:
            text = (f"💰Your balance: {pul[1]} sum\n\n"
f"Use the /bye command to top up your account.")
            await callback.message.answer(text, parse_mode="Markdown")



@router.message(StateFilter(ReferatState_en_kurs.title))
async def get_title(message: Message, state: FSMContext):
    await state.update_data(title=clean_text_ru(message.text))
    await message.answer("Enter the institute and faculty:")
    await state.set_state(ReferatState_en_kurs.institute)


@router.message(StateFilter(ReferatState_en_kurs.institute))
async def get_institute(message: Message, state: FSMContext):
    await state.update_data(institute=clean_text_ru(message.text))
    await message.answer("Enter the author (First Name Last Name):")
    await state.set_state(ReferatState_en_kurs.author)


@router.message(StateFilter(ReferatState_en_kurs.author))
async def get_author(message: Message, state: FSMContext):
    await state.update_data(author=clean_text_ru(message.text))
    await message.answer("Select the number of pages of the main text",reply_markup=sahifalar_soni())
    await state.set_state(ReferatState_en_kurs.pages)


@router.callback_query(ReferatState_en_kurs.pages)
async def get_pages(message: CallbackQuery, state: FSMContext):
    ...
    if int(check[3]) < tek:
        await bot.send_message(chat_id=message.from_user.id,text=
            f"There are not enough funds in your account 🙅!\Your account:💰 {check[3]}\n\n Click the /bye button to top up your account.",reply_markup=sahifalar_soni)
    else:

        await state.update_data(pages=pages)
        a=await message.message.answer("What language should this be written in?", reply_markup=language_keyboard_ru())
        await state.update_data(m_id=a.message_id)
        await state.set_state(ReferatState_en_kurs.language)


@router.callback_query(F.data.startswith("langenkurs_"))
async def get_language(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data({"m_id": a.message_id})
    await state.set_state(ReferatState_en_kurs.tanlov)


@router.callback_query(F.data == "ozgartirish", ReferatState_en_kurs.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data({"m_id": a.message_id})
    await state.set_state(ReferatState_en_kurs.ozgartirish)


@router.callback_query(F.data == "mavzu", ReferatState_en_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Submit author name:")
    await state.update_data({"mm_id": a.message_id})

    await state.set_state(ReferatState_en_kurs.mavzu)
    await callback.answer()
    print(2)


@router.callback_query(F.data == "til", ReferatState_en_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    a = await callback.message.answer("Choose language:", reply_markup=keyboard)
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_en_kurs.til)
    await callback.answer()
    print(1)


@router.callback_query(F.data == "muallif", ReferatState_en_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Submit author name:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_en_kurs.muallif)
    await callback.answer()
    print(3)


@router.callback_query(F.data == "institut", ReferatState_en_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Enter the institute and department:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_en_kurs.institutt)
    await callback.answer()
    print(4)


@router.callback_query(ReferatState_en_kurs.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    data = callback.data
...
@router.callback_query(F.data == "rad_etish", ReferatState_en_kurs.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
...
async def reja_manual(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(ReferatState_en_kurs.reja_manual)


@router.callback_query(F.data == "tayyorlash", ReferatState_en_kurs.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ol = await state.get_data()
    m_id = ol["m_id"]
    await bot.delete_message(chat_id=callback.from_user.id, message_id=m_id)
...
@router.callback_query(StateFilter(ReferatState_en_kurs.til))
async def handle_mavzu_message(message: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Science", callback_data="mavzu"),
                InlineKeyboardButton(text="Author ", callback_data="muallif")
    ...
    "🌟Great, check out the information below.\n\n"
    f"KURS ISHI\n"
    f"Topic: {mavzu}\n"
    f"Author: {ism}\n"
    f"Institute and department: {ins}\n"
...
@router.message(StateFilter(ReferatState_en_kurs.muallif))
async def handle_mavzu_message(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Object", callback_data="mavzu"),
                InlineKeyboardButton(text="Author ", callback_data="muallif")
    ...
    a = await bot.send_message(chat_id=message.from_user.id, text=
    "🌟Great, check the information below.\n\n"
    f"COURSE WORK\n"
    f"Object: {mavzu}\n"
    f"Author: {ism}\n"
    f"Institute and department: {ins}\n"
...
@router.message(StateFilter(ReferatState_en_kurs.mavzu))
async def handle_mavzu_message(message: Message, state: FSMContext):
...
@router.message(StateFilter(ReferatState_en_kurs.institutt))
async def handle_mavzu_message(message: Message, state: FSMContext):
    ol = await state.get_data()
    mm_id = ol.get("mm_id")
    await bot.delete_message(chat_id=message.from_user.id, message_id=mm_id)
...
@router.message(StateFilter(ReferatState_en_kurs.reja_manual))
...
async def generate_and_send_referat_ru(message: Message, state: FSMContext):
    ...
    try:
        ...
        caption = (f"📌 {data['title']}\n"
                   ...
                   f"💡 @Slaydai_bot\n\n"
                   f"To perform another action, press the /start button.")
...
async def send_short_version(message: Message, state: FSMContext):
    ...
    if not long_text:
        await message.answer("No text found to shorten.")
        return

    progress_msg = await message.answer("An abridged version is being prepared...")
