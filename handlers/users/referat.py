...
# --- FSM States ---
class ReferatState_uz_uz(StatesGroup):
    doc_type = State()
    title = State()
    institute = State()
    author = State()
...
def doc_type_keyboard_ru():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="REFERAT ", callback_data="set_type_uz_referat")],
        [InlineKeyboardButton(text="MUSTAQIL ISH", callback_data="set_type_uz_mustaqil")]
    ])
...
def language_keyboard_ru():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="О’zbek", callback_data="languz_uzbek"),
            InlineKeyboardButton(text="Русский", callback_data="languz_rus"),
            InlineKeyboardButton(text="English", callback_data="languz_english")
        ]
    ])
...
async def start_referat(callback: CallbackQuery, state: FSMContext):
    ...
    if olish:
        ...
        if int(summa[3]) >= tekshir:
            ...
            await state.set_state(RefesState_shablon_uz.title)
        ...
        if int(summa[3]) >= tekshir:
            ...
            if int(check[3]) < 2500:
                await callback.message.answer(
                    f"Hisobingizda yetarli mablag‘ yo‘q 🙅!\Hisobingiz:💰 {check[3]}\n\Hisobingizni to‘ldirish uchun /vidolashish kerak.")

            else:

                await state.set_state(ReferatState_uz_uz.doc_type)
...
@router.callback_query(F.data.startswith("set_type_uz_"))
async def set_doc_type(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Mavzuni kiriting:")
    await state.set_state(ReferatState_uz_uz.title)


@router.message(StateFilter(ReferatState_uz_uz.title))
async def get_title(message: Message, state: FSMContext):
    await state.update_data(title=clean_text_ru(message.text))
    await message.answer("Institut va fakultetga kiring:")
    await state.set_state(ReferatState_uz_uz.institute)


@router.message(StateFilter(ReferatState_uz_uz.institute))
async def get_institute(message: Message, state: FSMContext):
    ...
    await state.set_state(ReferatState_uz_uz.author)


@router.message(StateFilter(ReferatState_uz_uz.author))
async def get_author(message: Message, state: FSMContext):
    await state.update_data(author=clean_text_ru(message.text))
    await message.answer("Asosiy matnning sahifalar sonini kiriting (5-20-betlar):")
    await state.set_state(ReferatState_uz_uz.pages)


@router.message(StateFilter(ReferatState_uz_uz.pages))
async def get_pages(message: Message, state: FSMContext):

    if not message.text.isdigit():
        await message.answer("Iltimos, faqat raqamni kiriting (5-20):")
        return
    summa = db.select_summa()
    tek = int(message.text) * int(summa[1])
    pages = int(message.text)
    if not 5 <= pages <= 20:
        await message.answer("5 dan 20 gacha sahifalarni kiriting.")
        return
    ...
    if int(check[3]) < tek:
        ...
        a=await message.answer("Bu qaysi tilda yozilishi kerak?", reply_markup=language_keyboard_ru())
        await state.update_data(m_id=a.message_id)
        await state.set_state(ReferatState_uz_uz.language)


@router.callback_query(F.data.startswith("languz_"))
...
f"• Tugmani bosing «🚫 Rad etish», bekor qilish.",
    ...
    await state.update_data({"m_id": a.message_id})
    await state.set_state(ReferatState_uz_uz.tanlov)


@router.callback_query(F.data == "ozgartirish", ReferatState_uz_uz.tanlov)
...
f"Til: {til}\n\n",
    ...
    await state.update_data({"m_id": a.message_id})
    await state.set_state(ReferatState_uz_uz.ozgartirish)


@router.callback_query(F.data == "mavzu", ReferatState_uz_uz.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Отправить абстрактную тему:")
    await state.update_data({"mm_id": a.message_id})

    await state.set_state(ReferatState_uz_uz.mavzu)
    await callback.answer()
    print(2)


@router.callback_query(F.data == "til", ReferatState_uz_uz.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_uz_uz.til)
    await callback.answer()
    print(1)


@router.callback_query(F.data == "muallif", ReferatState_uz_uz.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Iltimos, muallifning ismini ko'rsating:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_uz_uz.muallif)
    await callback.answer()
    print(3)


@router.callback_query(F.data == "institut", ReferatState_uz_uz.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Institut va fakultetga kiring:")
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(ReferatState_uz_uz.institutt)
    await callback.answer()
    print(4)


@router.callback_query(ReferatState_uz_uz.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    data = callback.data
    ...
    if data == 'saqlash':
        ...
        a = await bot.edit_message_text(message_id=m_id,chat_id=callback.from_user.id, text=
        "🌟Ajoyib, quyidagi ma'lumotlarni tekshiring.\n\n"
        f"{doc_type}\n"
        f"Mavzu: {mavzu}\n"
        f"Muallif: {ism}\n"
...
@router.callback_query(F.data == "rad_etish", ReferatState_uz_uz.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
...
async def reja_manual(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(ReferatState_uz_uz.reja_manual)


@router.callback_query(F.data == "tayyorlash", ReferatState_uz_uz.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ol = await state.get_data()
    m_id = ol["m_id"]
    await bot.delete_message(chat_id=callback.from_user.id, message_id=m_id)
...
@router.callback_query(StateFilter(ReferatState_uz_uz.til))
...
@router.message(StateFilter(ReferatState_uz_uz.muallif))
...
@router.message(StateFilter(ReferatState_uz_uz.mavzu))
async def handle_mavzu_message(message: Message, state: FSMContext):
...
@router.message(StateFilter(ReferatState_uz_uz.institutt))
async def handle_mavzu_message(message: Message, state: FSMContext):
    ol = await state.get_data()
    mm_id = ol.get("mm_id")
    await bot.delete_message(chat_id=message.from_user.id, message_id=mm_id)
...
@router.message(StateFilter(ReferatState_uz_uz.reja_manual))
