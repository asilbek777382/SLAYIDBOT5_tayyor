...
# -------- STATES --------
class PresState_shablon__ru(StatesGroup):
    language = State()
    topic = State()
    author = State()
    bg = State()
...
class ImageCollectState_shablon__ru(StatesGroup):
    collecting = State()
...
@router.message(StateFilter(PresState_shablon__ru.topic))
async def get_topic(msg: Message, state: FSMContext):
    olish=db.select_shablon_pre(tg_id=msg.from_user.id,til='ru')
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Выбор изображения", callback_data="img_select_shab__ru")],
        [InlineKeyboardButton(text="♻️ Автоматические картинки", callback_data="img_auto_shab__ru")],
        [InlineKeyboardButton(text="🚫 Без картинки", callback_data="img_none_shab__ru")]
    ])
...
@router.callback_query(F.data == "img_select_shab__ru")
...
"🌃 Фотографий: 0/7",
    ...
    await state.set_state(ImageCollectState_shablon__ru.collecting)

@router.message(StateFilter(ImageCollectState_shablon__ru.collecting), F.photo)
async def collect_images(msg: Message, state: FSMContext):
    ...
    if len(user_images[uid]) >= 7:
        await msg.answer("❌ Вы уже отправили 7 фотографий!")
        return
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Заканчивать", callback_data="img_finish_preru_shab")],
        [InlineKeyboardButton(text="♻ Удалить фотографии", callback_data="img_reset_preru_shab")]
    ])
    await msg.answer(f"🌃 Фотографии: {len(user_images[uid])}/7", reply_markup=kb)

@router.callback_query(F.data == "img_reset_preru_shab")
...
@router.callback_query(F.data == "img_finish_preru_shab")
async def finish_images(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="✅ Подготовка", callback_data="tayyorlash_shab_preru"),
                InlineKeyboardButton(text="🚫 Отклонение", callback_data="rad_etish_shab_preru"),
                InlineKeyboardButton(text="✏️ Изменять", callback_data="ozgartirish_shab_preru")
            ]
        ]
    )
...
f"• Нажмите «🚫 Отклонить», чтобы отменить",
    ...
    await state.update_data({"m_id": a.message_id})
    await state.set_state(PresState_shablon__ru.tanlov)
    await callback.answer()

# --- Rasmsiz rejim ---
@router.callback_query(F.data == "img_none_shab__ru")
...
@router.callback_query(F.data == "img_auto_shab__ru")
...
@router.callback_query(F.data == "ozgartirish_shab_preru", PresState_shablon__ru.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Предмет", callback_data="mavzu_shab_preru"),
                InlineKeyboardButton(text="Автор ", callback_data="muallif_shab_preru"),
                InlineKeyboardButton(text="Язык", callback_data="til_shab_preru")
            ],
            [
                InlineKeyboardButton(text="-", callback_data="minus_shab_preru"),
                InlineKeyboardButton(text=f"Страницы: {requested_slide_count}", callback_data=f"count:{requested_slide_count}"),
                InlineKeyboardButton(text="+", callback_data="plus_shab_preru")
            ],
            [
                InlineKeyboardButton(text="⬅️", callback_data="oldingi_shab_preru"),
                InlineKeyboardButton(text=f"Дизайн {bg_num}", callback_data=f"dizayn:{bg_num}"),
                InlineKeyboardButton(text="➡️", callback_data="keyingi_shab_preru")
            ],
            [
                InlineKeyboardButton(text="Сохранять ✅", callback_data="saqlash_shab_preru"),
            ]
        ]
    )
    photo_manzili = Path(rf".\handlers\users\rasimlar\{bg_num}.jpg")
...
f"• Нажмите «🚫 Отклонить», чтобы отменить",
    ...
    await state.update_data({"m_id":a.message_id})
    await state.set_state(PresState_shablon__ru.ozgartirish)



@router.callback_query(F.data=="mavzu_shab_preru",PresState_shablon__ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    a = await callback.message.answer("Предложить тему презентации:")
    await state.update_data({"mm_id": a.message_id})

    await state.set_state(PresState_shablon__ru.mavzu)
    await callback.answer()
    print(2)
@router.callback_query(F.data=="til_shab_preru",PresState_shablon__ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data({"mm_id": a.message_id})
    await state.set_state(PresState_shablon__ru.til)
    await callback.answer()
    print(1)


@router.callback_query(F.data=="muallif_shab_preru",PresState_shablon__ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):

        a = await callback.message.answer("Укажите имя автора:")
        await state.update_data({"mm_id": a.message_id})
        await state.set_state(PresState_shablon__ru.muallif)
        await callback.answer()
        print(3)


@router.callback_query(PresState_shablon__ru.ozgartirish)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    """
    ...
    if data == 'saqlash_shab_preru':
        ...
        elif data == "minus_shab_preru":
            if int(requested_slide_count) < 6:
                await callback.answer("Вы можете выбрать не менее 5 страниц..❗️", show_alert=True)
                await state.set_state(PresState_shablon__ru.ozgartirish)
            else:
                user_data.setdefault(uid, {})["requested_slide_count"] = int(requested_slide_count) - 1
                await state.set_state(PresState_shablon__ru.ozgartirish)
        ...
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(text="Тема", callback_data="mavzu_shab_preru"),
                    InlineKeyboardButton(text="Автор", callback_data="muallif_shab_preru"),
                    InlineKeyboardButton(text="Язык", callback_data="til_shab_preru")
                ],
                [
...
f"Количество страниц: {requested_slide_count}\n"
f"Язык: {user_lang}\n\n"
"• Если информация верна, нажмите «✅ Подготовить»!\n"
"• Используйте кнопки выше, чтобы изменить"
            )
        )
...
@router.callback_query(F.data == "rad_etish_shab_preru", PresState_shablon__ru.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
...
@router.callback_query(StateFilter(PresState_shablon__ru.til))
async def handle_mavzu_message(message: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(text="-", callback_data="minus_shab_preru"),
                InlineKeyboardButton(text=f"Язык: {requested_slide_count}",
                                     callback_data=f"count:{requested_slide_count}"),
                InlineKeyboardButton(text="+", callback_data="plus_shab_preru")
            ],
            [
...
@router.message(StateFilter(PresState_shablon__ru.muallif))
...
@router.message(StateFilter(PresState_shablon__ru.mavzu))
async def handle_mavzu_message(message: Message, state: FSMContext):
...
@router.callback_query(F.data == "tayyorlash_shab_preru", PresState_shablon__ru.tanlov)
async def tayyorlash_handler(callback: CallbackQuery, state: FSMContext):
    ol=await state.get_data()
    m_id=ol["m_id"]
    await bot.delete_message(chat_id=callback.from_user.id,message_id=m_id)
